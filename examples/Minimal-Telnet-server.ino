...
// #include "servers/ftpClient.h"       // #including ftpClient.h prior to #including telnetSerevr.hpp brings support for:   
                                        //          ftpput, ftpget
...
String telnetCommandHandlerCallback (int argc, char *argv [], telnetConnection *tcn) {  // handle the Telnet command and send a reply back to the Telnet client
...
telnetServer *myTelnetServer = NULL;
...
void setup () {
    ...
    myTelnetServer = new telnetServer (telnetCommandHandlerCallback); // there are other arguments available to handle WebSockets, firewall, ... if telnetServer will only serve already built-in commands this argument can be omitted
    if (myTelnetServer && myTelnetServer->state () == telnetServer::RUNNING)
