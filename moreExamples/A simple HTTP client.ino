...
void setup () {
  ...
  // wait until IP is obtained from reouter
  while (WiFi.localIP ().toString () == "0.0.0.0") { delay (1000); Serial.printf ("   .\n"); }

  // get remote web page
  String httpReply = httpClient ((char *) "github.com",                                      // server or IP
                                 80,                                                         // defaut HTTP port
                                 (char *) "/BojanJurca/Esp32_web_ftp_telnet_server_template" // page address
                                );
