            ...
            public:
                ...
                int recv (void *buf, size_t len) {
                    ...
                    switch (receivedThisTime) {
                        case -1:
                                  #ifdef __DMESG__
                                      dmesgQueue << "[tcpConn] recv error: " << errno << " " << strerror (errno);
                                  #endif
                                  return -1;
                        case 0:
                                  #ifdef __DMESG__
                                      dmesgQueue << "[tcpConn] closed by peer";
                                  #endif
                                  return 0;
                    }
                ...
                int sendBlock (void *buf, size_t len) {
                    ...
                    while (sentTotal != len) {
                        toSendThisTime = len - sentTotal; if (toSendThisTime > 1440) toSendThisTime = 1440; // 1440 is the largest payload that fits into one packet: MTU = 1500, 20 bytes are used for TCP header and 40 for IPv6 header (for IPv4 sligltly less, but we want a general solution), TCP_SND_BUF = 5744 (maximum ESP32 can send in one row)
                        sentThisTime = send (__connectionSocket__, ((char *) buf) + sentTotal, toSendThisTime, 0);
                        switch (sentThisTime) {
                            case -1:
                                      #ifdef __DMESG__
                                          dmesgQueue << "[tcpConn] send error: " << errno << " " << strerror (errno);
                                      #endif
                                      return -1;
                            case 0:
                                      #ifdef __DMESG__
