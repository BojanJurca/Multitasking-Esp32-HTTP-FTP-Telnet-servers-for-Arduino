 ...
 */
  ...
  String __smtpReply__ (TcpConnection *connection) {
    char buffer [256]; *buffer = 0;                                                            
    int receivedTotal = 0;
    int received;
    while ((received = connection->recvData (buffer + receivedTotal, sizeof (buffer) - receivedTotal - 1))) {
  ...
  String __sendMail__ (String message, String subject, String to, String from, String password, String userName, int smtpPort, String smtpServer) {
    ...
    TcpClient smtpClient (smtpServerIP.toString (), smtpPort, (TIME_OUT_TYPE) 5000); 
    TcpConnection *connection = smtpClient.connection ();
    if (!connection) return "Could not connect to " + smtpServer + " ("+ smtpServerIP.toString () + ") on port " + String (smtpPort) + ".";
    // debug: Serial.println ("[SMTP] connected."); 
  
    // 3. read welcome message from SMTP server
    r = __smtpReply__ (connection); if (r.substring (0, 3) != "220") { connection->closeConnection (); return "SMTP error: " + r; }
    
    // 4. send EHLO
    connection->sendData ("EHLO " + String (HOSTNAME) + "\r\n");
    r = __smtpReply__ (connection); if (r.substring (0, 3) != "250") { connection->closeConnection (); return "SMTP error: " + r; }
  
    // 5. send login request
    connection->sendData ((char *) "AUTH LOGIN\r\n");
    r = __smtpReply__ (connection); if (r.substring (0, 3) != "334") { connection->closeConnection (); return "SMTP error: " + r; }
    ...
    connection->sendData ((char *) encoded);
    r = __smtpReply__ (connection); if (r.substring (0, 3) != "334") { connection->closeConnection (); return "SMTP error: " + r; }
    mbedtls_base64_encode (encoded, sizeof (encoded) - 3, &encodedLen, (const unsigned char *) password.c_str (), password.length ());
    strcpy ((char *) encoded + encodedLen, "\r\n");
    connection->sendData ((char *) encoded);
    r = __smtpReply__ (connection); if (r.substring (0, 3) != "235") { connection->closeConnection (); return "SMTP error: " + r; }
      ...
      switch (from.charAt (i)) {
        ...
        case '@':   if (!quotation) { // we found @ in email address
                      ...
                      int k; for (k = i + 1; k < from.length () && from.charAt (k) > '\"' && from.charAt (k) != ','; k ++); // go forward to the end of email address
                        ...
                        connection->sendData ("MAIL FROM:" + from.substring (j + 1, k) + "\r\n");
                        r = __smtpReply__ (connection); if (r.substring (0, 3) != "250") { connection->closeConnection (); return "SMTP error: " + r; }
      ...
      switch (to.charAt (i)) {
        ...
        case '@':   if (!quotation) { // we found @ in email address
                      ...
                      int k; for (k = i + 1; k < to.length () && to.charAt (k) > '\"' && to.charAt (k) != ','; k ++); // go forward to the end of email address
                        connection->sendData ("RCPT TO:" + to.substring (j + 1, k) + "\r\n");
                        r = __smtpReply__ (connection); if (r.substring (0, 3) != "250") { connection->closeConnection (); return "SMTP error: " + r; }
    ...
    // 9. DATA
    connection->sendData ((char *) "DATA\r\n");
    r = __smtpReply__ (connection); if (r.substring (0, 3) != "354") { connection->closeConnection (); return "SMTP error: " + r; }
