...
*/
    ...
    */
    ...
    void delayMicrosecondsUntil (unsigned long *lastTime, unsigned long period) {
        ...
        if (nextTime - now < period) {
            // DEBUG: Serial.println (nextTime - now);
            delayMicroseconds (nextTime - now);
        }
        *lastTime = now;
    }
    inline void delayMicrosecondsUntil (unsigned long *lastTime, unsigned long period, unsigned long now) __attribute__((always_inline));
    void delayMicrosecondsUntil (unsigned long *lastTime, unsigned long period, unsigned long now) {
        unsigned long nextTime = *lastTime + period;
        if (nextTime - now < period) {
            // DEBUG: Serial.println (nextTime - now);
            delayMicroseconds (nextTime - now);
